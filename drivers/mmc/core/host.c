 ...
 * power of system pull up -- load driver of wifi -- wifi detect card by itself
 * because wifi can detect card by itself when power pull up,
 * so we need mmc rescan operation neglect the wifi host. */

/* get structure of "host" for wifi detecting card.
 * the function is called by mmc_add_host() and mmc_remove_host() */
static int select_sdio_host(struct mmc_host *host, int add)
{

	//printk("CG: %s: host index=%d\n", __FUNCTION__, host->index);	
...
/* wifi detect card by itself.
 * the function is called by bcm_wifi_set_carddetect() in board-msm8960-wifi.c.
 * the function is used in wifi driver at last. */
