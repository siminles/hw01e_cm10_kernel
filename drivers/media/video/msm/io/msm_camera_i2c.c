	...
	enum msm_camera_i2c_data_type data_type)
{
	...
	for (i = 0; i < size; i++) {
		enum msm_camera_i2c_data_type dt;
        //just for delay times
        if(( 0 != reg_conf_tbl->mdelays)
...
int32_t msm_sensor_write_conf_array_delay(struct msm_camera_i2c_client *client,
			struct msm_camera_i2c_conf_array *array, uint16_t index)
{
	int32_t rc;

	rc = msm_camera_i2c_write_tbl_delay(client,
		(struct msm_camera_i2c_reg_conf_delay*) array[index].conf,
		array[index].size, array[index].data_type);
			...
			struct msm_camera_i2c_conf_array *array, uint16_t size)
{
	int32_t rc = 0, i;
	for (i = 0; i < size; i++) {
		rc = msm_sensor_write_conf_array_delay(client, array, i);
		if (rc < 0)
			break;
	}
	return rc;
