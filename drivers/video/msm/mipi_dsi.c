...
void mipi_dsi_phy_reinit(int panel_ndx, struct msm_panel_info const *panel_info,
	int target_type);
static int mipi_video_panel_dsi_mdp_reinit(struct msm_fb_data_type *mfd)
{
    int ret = 0;
    ...
    mipi_dsi_phy_pll_config(mfd->panel_info.clk_rate);
...
int mipi_video_restore_default_fps(void)
{
    ...
    mfd = platform_get_drvdata(this_device);
    panel_data = this_device->dev.platform_data;
    /* default is set to 60FPS to follow frameworks rate */
    mfd->panel_info.clk_rate =panel_data->panel_info.clk_rate;
...
int mipi_video_set_low_fps(int frame_rate)
{
    ...
    mfd = platform_get_drvdata(this_device);
    /* set low rate to 30FPS and do not override a default clock rate */
...
static ssize_t framerate_show(struct device *pdev,
                 struct device_attribute *attr, char *buf)
{
    ...
    printk("%s frame_rate:%d\n",__func__,mfd->panel_info.mipi.frame_rate);
    count = sprintf(buf, "%u\n", mfd->panel_info.mipi.frame_rate);

    return count;
}

static ssize_t framerate_store(struct device *pdev,
                  ...
                  const char *buf, size_t count)
{
    ...
    mfd = platform_get_drvdata(this_device);
    if (strict_strtoul(buf, 10, &frame_rate))
        return -EINVAL;
    if (!frame_rate)
        return -EINVAL;
    mfd->panel_info.mipi.frame_rate = frame_rate;
...
static int mipi_dsi_clk_reinit(struct msm_panel_info *pinfo)
{
	...
	if (pinfo->type == MIPI_VIDEO_PANEL &&
		!pinfo->clk_rate) {
		h_period += pinfo->mipi.xres_pad;
		v_period += pinfo->mipi.yres_pad;
