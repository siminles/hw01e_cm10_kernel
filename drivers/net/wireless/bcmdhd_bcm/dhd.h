...
#define DHD_IOVAR_LENGTH_SOFTAP_CONFIG 256

typedef struct dhd_softap_configuration {
	uint32 security_flag;
	uint32 config_done_flag;
	char ssid[DHD_IOVAR_LENGTH_SOFTAP_CONFIG];
	uint32 ssid_len;
	char wsec[DHD_IOVAR_LENGTH_SOFTAP_CONFIG];
	uint32 wsec_len;
	char auth[DHD_IOVAR_LENGTH_SOFTAP_CONFIG];
	uint32 auth_len;
	char wpa_auth[DHD_IOVAR_LENGTH_SOFTAP_CONFIG];
	uint32 wpa_auth_len;
	char wsec_key[DHD_IOVAR_LENGTH_SOFTAP_CONFIG];
	uint32 wsec_key_len;
} dhd_softap_configuation_t;
...
/* Default KEEP_ALIVE Period is 45 sec to prevent AP from sending Keep Alive probe frame */
/* There is a AP which disconnect 60 seconds if no data, so we would like to implement 45 sec. 
If 55 sec, the buffer is only 5 sec, so we assume that sometimes intervel over 60 seconds.*/
#define KEEP_ALIVE_PERIOD 45000
