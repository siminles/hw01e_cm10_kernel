...
wl_cfg80211_del_virtual_iface(struct wiphy *wiphy, struct net_device *dev)
{
	...
	rtnl_unlock();
	msleep(1000); /* sleep more time than 500 */
	rtnl_lock();
...
wl_cfg80211_notify_ifdel(struct net_device *ndev)
{
	...
	if (p2p_is_on(wl) && wl->p2p->vif_created &&
		...
		WL_DBG(("IF_DEL event received, net %x, vif name: %s\n",
	...
	bool enabled, s32 timeout)
{
    ...
    /* 
       When it is only p2p interface, do not enable the power save;
       or else power consumption will increase 50mA in HOME when wifi connect AP.
   */
	...
	// Make sure P2P is not in PowerSave mode
...
static s32 __wl_cfg80211_up(struct wl_priv *wl)
{
	...
	WL_TRACE(("In\n"));
/* Remove arp offload to avoid ping failed */
