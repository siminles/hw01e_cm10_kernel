	...
	for (pnext = pkt; pnext; pnext = PKTNEXT(sd->osh, pnext)) {
		...
		if ((write) && (!fifo)) {
			err_ret = sdio_memcpy_toio(gInstance->func[func], addr,
				buf ? buf : ((uint8*)PKTDATA(sd->osh, pnext)),
				pkt_len);
		} else if (write) {
			err_ret = sdio_memcpy_toio(gInstance->func[func], addr,
				buf ? buf : ((uint8*)PKTDATA(sd->osh, pnext)),
                     ...
                     uint addr, uint reg_width, uint buflen_u, uint8 *buffer, void *pkt)
{
	...
	if (pkt == NULL) {
		...
		Status = sdioh_request_packet(sd, fix_inc, write, func, addr, mypkt, buffer);
		...
		Status = sdioh_request_packet(sd, fix_inc, write, func, addr, mypkt, NULL);
		...
		Status = sdioh_request_packet(sd, fix_inc, write, func, addr, pkt, NULL);
	}
